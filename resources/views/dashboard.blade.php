    ...
    <div class="min-h-screen flex flex-col md:flex-row">
        ...
        <div class="flex-1 p-4 md:p-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between bg-white shadow-md p-4 mb-6 space-y-3 sm:space-y-0">
                <div>
                    <h1 class="text-2xl md:text-3xl font-semibold text-gray-800">Dashboard Overview</h1>
                    <p class="text-gray-500 text-sm md:text-base">Monitor your API integration and sandbox testing progress</p>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mt-6">
                <div class="p-4 md:p-6 rounded-lg border-2 border-red-500 bg-white">
                    <h2 class="text-xl md:text-2xl font-semibold">{{ $data['total_tests'] }}</h2>
                ...
                <div class="p-4 md:p-6 rounded-lg border-2 border-green-500 bg-white">
                    <h2 class="text-xl md:text-2xl font-semibold">{{ $data['success_rate'] }}</h2>
                    <p class="text-sm">Success Rate</p>
                </div>

                <div class="p-4 md:p-6 rounded-lg border-2 border-blue-500 bg-white">
                    <h2 class="text-xl md:text-2xl font-semibold">{{ $data['active_apis'] }}</h2>
                ...
                <div class="p-4 md:p-6 rounded-lg border-2 border-indigo-500 bg-white">
                    <h2 class="text-sm md:text-2xl font-semibold break-words">{{ $data['last_test_date'] }}</h2>
            ...
            <div class="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div class="flex items-center space-x-4">
                                <div class="w-full p-4 border rounded-lg {{ $test['result'] == 'success' ? 'bg-green-50' : ($test['result'] == 'failed' ? 'bg-red-50' : 'bg-yellow-50') }}">
                                    <div class="flex items-center space-x-2">
                                        ...
                                        @endif
                                        <h3 class="text-base md:text-lg">{{ $test['name'] }}</h3>
                                    </div>
                                    <p class="text-xs md:text-sm mt-1">{{ $test['status'] }} - {{ $test['time'] }}</p>
                ...
                <div class="bg-white shadow-lg rounded-lg p-4 md:p-6">
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                        <h2 class="text-xl md:text-2xl font-semibold">Active API Services</h2>
                        <!-- View All Button -->
                        <button class="text-blue-600 hover:text-blue-800 text-sm md:text-base">View All</button>
                    </div>
                    <div class="space-y-4 mt-4">
                        @foreach($data['active_api_services'] as $service)
                            <div class="flex items-center space-x-4">
                                <div class="w-full p-4 border rounded-lg bg-blue-100">
                                    <h3 class="text-base md:text-lg">{{ $service['name'] }} (v{{ $service['version'] }})</h3>
                                    <p class="text-xs md:text-sm mt-1">
                                        {{ $service['calls'] }} calls - Status: 
            ...
            <div class="mt-6">
                <button class="bg-red-500 text-white py-2 px-4 md:px-6 rounded-lg hover:bg-red-600 w-full sm:w-auto">
